...
public class GestionBonos {
		...
		boolean valNoVal = true;
			...
			while (valNoVal) {
				try {
					...
					System.out.println("Introduce un entero válido (" + NUM_SESIONES1 + ", " + NUM_SESIONES2 + ", "
							+ NUM_SESIONES3 + " o " + NUM_SESIONES4 + ")");
				} 
			}
		return cantSesion;
	...
	private static String solicitarDNI() {
		...
		while (num != 9) {
			...
			if (num != 9) {
				System.out.println("Introduce un valor válido NNNNNNNNX o " + CLAVE_FIN + "para finalizar el programa");
			}
		}
